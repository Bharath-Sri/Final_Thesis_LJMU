...
const ChatPopup = ({ onClose }) => {
  ...
  //   const feedback_button = ChatComponent(feedback, index);
  //   // Handle the feedback submission logic here
  // };

  const handleFeedback = async (feedback, index) => {
    try {
      // Assuming ChatComponent is a function that sends feedback to the server
      const response = await ChatComponent(feedback, index);
                ...
                <Box key={index} direction="column" align={message.sender === 'user' ? 'end' : 'start'} pad="small" gap="small">
                  ...
                  {message.sender === 'bot' && (
                    <Box direction="row" gap="small" justify="end" align="center">
                      <Button icon={<Like />} onClick={() => handleFeedback('Like', index)} />
                      <Button icon={<Dislike />} onClick={() => handleFeedback('Dislike', index)} />
                    </Box>
                  )}
                </Box>
              ))}
